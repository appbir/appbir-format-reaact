...
export let connect = model=> {
    let {moduleName,initState,initProps, stateToProps=(state)=>state ,...actionToProps} = model;    
...
export let injectionReducer = (_moduleName, _mapStateToProps, _mapDispatchToProps, _reducer, _mergeProps) => {
    ...
    return function injectionWithConnect(WrappedComponent) {
        class InjectionConnect extends Component {
            ...
            render() {
                ...
                if (reducer) {
                    ...
                    Logger.frame('info','replaceReducer',moduleName,namespace);
                    frame.store.replaceReducer(combineModuleReducer(newReducer));
                }
                let { initProps , __mergeProps}= mergeProps;
                let InjectionComponent = reduxWithConnect(mapStateToProps, mapDispatchToProps, 
                    __mergeProps, { moduleName, namespace,props:this.props, initProps })(WrappedComponent);

                let instance = createElement(InjectionComponent,this.props,this.props.children);
                this.instance = instance;
                debugger;
                return instance;
                // return ( <InjectionComponent {...this.props} /> )
            }
        }
        return InjectionConnect;
    }
}
