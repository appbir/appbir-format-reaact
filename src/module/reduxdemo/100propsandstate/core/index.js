...
export let connect = model=> {
    ...
    Logger.frame('error','connect namespace',namespace);
...
let reduxWithConnect = (mapStateToProps, mapDispatchToProps, mergeProps, option) => {
     ...
     * 2: 获取指定模块   getState(namespace)
     * 4: 获取指定区域 getState(false,areaName)  // 获所有模块的数据  // 后续完善
    ...
    return function wrapWithConnect(WrappedComponent) {
        class FrameConnect extends Component {
            constructor(props){
                super(props)
                this.props.namespace && recordNamespace(this.props.namespace,_namespace);
            }
            shouldComponentUpdate = nextProps=>{
                Logger.frame('info','wrapWithConnect shouldComponentUpdate',_namespace)
