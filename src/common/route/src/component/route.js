...
class LoadComponent extends Component {
    ...
    // 加载页面loading
    render=()=> this.state.loadComponent ? <div>{this.props.children}</div> : <div className="loading-page">加载ing</div>;
  }
...
class AuthFilter extends Component {
  ...
  // 根据权限进行渲染组件是无权限页面
  render=()=> isPermit ? <div>{this.props.children}</div> : <div>无权限</div>;
}
    ...
    return (<ReactDOMRoute component={(pro)=> (
              ...
              <AuthFilter {...prop}>
                  ...
                  <LoadComponent load={component}>
                        {/* 初始化构建组件 */}
                        <Container  {...prop} {...pro} auth={state.auth}/>
                  </LoadComponent>
              </AuthFilter>
              )} {...prop}/>)
}
